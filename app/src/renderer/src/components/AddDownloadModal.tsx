...
export function AddDownloadModal({ initialUrl = "", onClose, onStarted }: AddDownloadModalProps) {
  ...
  useEffect(() => {
    window.kitsune.getDownloadsDir().then(dir => {
      setSavePath(dir);
    }).catch(() => {});
  }, []);
  ...
  const fetchMetadata = async (targetUrl: string) => {
    ...
    setError("");
    try {
      const meta = await window.kitsune.getMetadata(targetUrl);
      setMetadata(meta);
      setFilename(meta.filename);
      const dir = await window.kitsune.getDownloadsDir();
  ...
  const handleStart = async () => {
    ...
    try {
      await window.kitsune.startDownload({
        downloadId,
        url,
        path: savePath,
        connections,
